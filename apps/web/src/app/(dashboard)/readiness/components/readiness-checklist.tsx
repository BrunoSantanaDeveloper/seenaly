 ...
 *
 * Each group carries one line of WHY it matters (teaching where the user is
 * already looking) and a confirmed/total counter — completion drive over real
 * setup state, never points or badges.
 */
          ...
          return (
            <Box key={group.key} className="flex flex-col gap-2">
              ...
              <Box className="flex flex-col pl-1">
                ...
                {/* Where the money is actually taken — a select, because "none"
                    is a real and blocking answer, not an unchecked box. */}
                {group.key === "checkout" && (
                  <Box className="mt-2 flex flex-row flex-wrap gap-3">
                    ...
                    {profile.hasGuarantee && (
                      <TextField
                        ...
                        disabled={disabled}
                        onChange={(event) =>
                          set("guaranteeDays", event.target.value === "" ? null : Number(event.target.value))
                        }
