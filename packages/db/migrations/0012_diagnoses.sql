...
HIPÓTESES CONCORRENTES: quando o gargalo é pós-clique, separar PÁGINA (a pessoa não avança) de CHECKOUT (avança e abandona) de PREÇO/OFERTA (avança e desiste no valor) depende dos dados de funil. Se o briefing trouxer a seção "Funil e vendas reais" com números, USE-A para discriminar: queda forte de visitas→checkout aponta a PÁGINA; queda forte de checkout→compra aponta CHECKOUT/OFERTA/PREÇO; taxa de reembolso alta aponta expectativa desalinhada/entrega. Se NÃO houver dados de funil, nomeie as hipóteses concorrentes, escolha a mais provável com justificativa, e peça em missing_data exatamente o dado que as discrimina (ex.: taxa de checkout iniciado / compras). Preencha missing_data MESMO quando insufficient_data=false e a confiança for alta. Não afirme "o problema é a página" quando os dados disponíveis também comportam "o problema é o checkout".
