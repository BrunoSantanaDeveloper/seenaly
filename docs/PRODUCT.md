...
- **Oferta e preço** — clareza da promessa, equação de valor, garantia, ancoragem, ticket vs. teto do público frio (reusa o modelo de cobrança já capturado no cadastro). Auditada a partir do modelo de contexto do produto (promessa, preço, planos, provas, garantia declarada no intake), por desenho sem grupo próprio no checklist: a porta de correção é a tela de contexto, e o veredito seguinte relê o contexto atualizado.
- **Página e message match** — existe página; a promessa da página bate com a do anúncio; prova, estrutura, CTA. O lado do anúncio do message match NÃO é um campo declarado no intake: ele vem das promessas/ganchos registrados na biblioteca de criativos (e, antes disso, das hipóteses do Plano de Teste Criativo, fase 8); enquanto não existem criativos, o motor declara o dado ausente em vez de pedir uma promessa hipotética.
...
- **Ativação (somente trial-first)** — fricção do cadastro, momento de ativação definido e medido, conversão trial → pagante, caminho de upgrade. Existe apenas quando o modelo de aquisição declarado é trial-first (dimensão `ativacao` do veredito, migração 0034).
...
- **Prontidão de mídia** — criativos mínimos por ângulo. A *estrutura* da 1ª campanha (objetivo, conjuntos, orçamento, segmentação) **não** é auditada aqui: é prescrição de plataforma e pertence ao Plano de Lançamento (fase 9). A Prontidão audita a estrutura que o negócio possui; o que se configura dentro do Gerenciador é sempre próximo passo, nunca tarefa desta tela.
...
- **Enriquecido (scan opcional)** — um scanner técnico da URL (title/meta/OG/canonical/sitemap/robots/dados estruturados, presença de pixel no HTML, peso e tempo de resposta) vira **evidência observada**; é enriquecimento no mesmo padrão do conector Meta — valor nunca fica atrás do scan. O scan **não escreve no checklist** (a leitura auto-confirma apenas o que PROVOU — um caminho de mão única que nunca desmarca nada): onde declarado e observado divergem, a divergência é o achado mais valioso do veredito. Quatro limites são declarados ao motor e ao usuário, porque cada um viraria achado falso se escondido: página renderizada no cliente esconde tags de um fetch de HTML; **um Pixel carregado via Google Tag Manager é invisível ao HTML inicial** (GTM presente ⇒ ausência de tag nunca é prova, nunca contestação); **CAPI é server-side e invisível a qualquer scan**; a leitura do robots.txt é aproximada. Itens cuja prova é estruturalmente inatingível (SPA/GTM) assentam como *declarados e inverificáveis* — nunca ficam "aguardando prova" para sempre. O tempo da nossa requisição não é Core Web Vitals; a velocidade OFICIAL vem do **PageSpeed Insights** (opcional, `GOOGLE_PAGESPEED_API_KEY`; sem a chave, nada muda), que dá prova real ao item "página carrega rápido" nos cortes do próprio Google (LCP ≤ 2,5s bom, > 4s ruim).

**Todo achado diz ONDE a correção acontece (migração 0043).** Um veredito que não localiza a correção não é acionável — e, pior, erra em silêncio. O caso real que originou a regra: um SaaS trial-first recebeu *"adicione uma garantia clara à sua oferta e exiba-a na página de destino"* enquanto o herói do site já dizia *"14 dias grátis, sem cartão · cancele quando quiser"*; o que de fato faltava estava em **outra tela** — os planos pagos não declaravam reembolso nem cancelamento. Dimensão certa, superfície errada. O mesmo achado ainda invocava aversão à perda sobre o **cadastro gratuito**, onde não há dinheiro em jogo.

Por isso cada finding agora carrega dois campos obrigatórios no schema: **`stage`** — a etapa da jornada em que o problema custa dinheiro, num enum **gerado do modelo de funil declarado** (trial-first tem `upgrade` e `cadastro`; venda direta tem `checkout` e nenhum dos dois; lead-first tem `atendimento` e nenhum checkout) — e **`where`**, o localizador concreto da tela ("card do plano Pro na página de planos, abaixo do preço"), com "na sua página" explicitamente proibido quando o negócio tem mais de uma superfície. O enum é o mesmo padrão de `related_items` (0042) e dos slugs do plano criativo: enumeração derivada do código, imposta pelo provedor, impossível de derivar. Os dois campos são opcionais no *tipo* — vereditos gravados antes continuam validando e renderizando.

Duas regras de honestidade vêm junto, porque o campo sozinho convidaria ao chute: o motor **só conhece as telas que lhe contaram** (o contexto tem UMA url), então uma correção que mora noutra tela deve pedir a url em `missing_data` em vez de ser empurrada para a página de destino; e ele **não conclui sobre o que não leu** — critério de sucesso que é fato observável numa tela, sem evidência de scan daquela tela, vira pedido de leitura, nunca afirmação de ausência.

> **Estudo aberto — mapa de superfícies.** A limitação de fundo é o contexto ter uma única `landingPageUrl`. Declarar as demais superfícies (planos, checkout/upgrade, cadastro) destravaria três coisas de uma vez: o scan lendo mais de uma página e produzindo evidência **observada** por superfície; o `where` deixando de depender do que o usuário contou; e o *message match* virando comparação real entre a promessa do anúncio, a da página e a dos planos — que o texto acima ainda admite como manco. Custo: migração + UI de intake + scan multi-página. Não implementado.

**O laço se fecha nos experimentos.** Cada achado do veredito vira, sob demanda, um experimento rastreado no journal existente — por **achado**, não por veredito, porque um veredito carrega até sete correções independentes e juntá-las destruiria justamente o aprendizado que a memória existe para guardar. A idempotência usa `(diagnosis_id, change_made)`: a ação recomendada já identifica o achado, então clicar duas vezes reaproveita o experimento. Como todo experimento concluído volta ao briefing — o brief de prontidão inclui a memória de experimentos, com a instrução explícita de nunca re-recomendar correção concluída —, uma correção estrutural feita hoje deixa de ser re-recomendada amanhã. E o laço tem cadência: todo veredito grava `next_review_at` (o modelo propõe em [1, 60] dias; o servidor garante com fallback 7/14/30 por veredito), e o cron de lembretes — ciente de escopo, para que um diagnóstico de campanha mais novo nunca silencie uma releitura de prontidão devida — chama o usuário de volta.

**Do "o quê" ao "como" — passo a passo sob demanda (migração 0031).** Cada achado gera, sob demanda, um passo a passo aterrado na base de conhecimento (assistente `readiness-howto`): pago por achado, cacheado por `(diagnosis_id, finding_index)` — pedir duas vezes nunca cobra duas vezes, garantia hoje transacional (`record_readiness_howto_and_charge`, migração 0039) — e honesto com teto: quando a base não sustenta o tutorial, o motor devolve passos vazios, nada é cobrado e nada é cacheado, para que uma tentativa após a base crescer possa funcionar.

**Concierge — a saída humana paga, merecida e nunca permanente (migração 0032).** Quando nem o passo a passo resolve, existe a sessão guiada (`assist_offerings` + fila `readiness_assists`). Invariante: a oferta só aparece depois de resistência real naquele item (afirmação contestada pelo scan após nova tentativa; item de especialista pulado depois de ler a explicação; ensinado + escaneado e ainda não comprovado — sinais persistidos em `product_readiness.extra.journey`, para que a oferta merecida não evapore num reload) e nunca para item DIY, já comprovado, já pedido ou inaplicável — caso contrário o diagnóstico viraria funil de upsell competindo com o caminho gratuito. O preço é declarado antes do clique, pedido e cobrança são UMA transação (`record_assist_and_charge`, migração 0038) e nunca pedimos senhas.

**Modelo de aquisição declarado (migração 0034).** O usuário declara `direct`, `trial_first` ou `lead_first`; o briefing gerado — nunca o prompt do operador — reenquadra o que cada dimensão audita. Trial-first ganha o grupo de ativação (4 itens declarados, atrás do login e invisíveis a qualquer scan), o bloqueador trial → pagante não medido e a dimensão `ativacao` do veredito; lead-first remove as exigências de checkout self-service.

**Cobrança é transacional por construção.** Persistir e cobrar são UMA transação (`record_diagnosis_and_charge`, migração 0038): o usuário paga se e somente se o veredito existe; um débito falho desfaz o insert, um insert falho não cobra nada. Um lock por produto (migração 0040, TTL 180s, fail-open) impede que duas abas paguem por dois vereditos, e o scan gratuito tem cooldown de 60s por produto — contenção de recurso, nunca portão de valor.
...
## Plano de Lançamento — do "pode anunciar" ao "como gastar os primeiros reais"

Entre a Prontidão (fase 7) e o diagnóstico de campanha (fase 3) existe um **penhasco**. O diagnóstico só funciona quando já há gasto para ler; a Prontidão termina dizendo "sua estrutura está pronta" — e o produto cala exatamente no momento de maior risco financeiro do iniciante. Não é a página que queima o orçamento de quem nunca anunciou: é a **primeira semana de gasto**. Evento de otimização errado, oito conjuntos de R$20/dia que nunca saem da fase de aprendizado, conjunto morto no segundo dia, edição no meio do aprendizado que reseta a contagem. Se o Seenaly fica mudo aí, ele não apenas deixa de entregar — ele **leva a culpa**: "fiz tudo o que ele mandou e queimei mesmo assim". E o usuário que precisa aprender sozinho estrutura de campanha, orçamento e leitura de aprendizado é, por definição, um usuário que não precisa do Seenaly.

O penhasco é estrutural, e ele já vazava: a Prontidão carregava duas preocupações que não são estrutura e que ela não consegue entregar — o item `remarketingAudience` na dimensão `funil` (um público de remarketing mora dentro do Business Manager e só existe depois de tráfego acumulado — pedi-lo *antes* do primeiro real gasto pressupõe conta e tráfego que ainda não existem, contra o invariante de maturidade) e "estrutura mínima para a primeira campanha" na dimensão `midia`. Eram sintomas da fase que faltava.

O **Plano de Lançamento** é o mesmo motor apontado para um terceiro alvo: `scope = 'launch_plan'` em `diagnoses`, mesmo formato estruturado, mesma honestidade (`insufficient_data` / `missing_data`), grounding em `meta-ads-docs` (trust 1, mecânica de plataforma) + `growth-playbook`. Nenhum domínio novo, nenhuma tabela nova.

A pergunta que ele responde: *"com a estrutura que você provou ter, os criativos que você tem e a economia que você declarou — qual é a **menor aposta que produz aprendizado confiável**, e o que você não deve tocar enquanto ela roda?"*

O que o torna defensável e não conselho genérico é que **cada número sai de dado que o produto já tem**:

- **O evento de otimização** vem do que a Prontidão *provou* que dispara — não de um palpite. A fase 7 é input da fase 9: recomendar otimizar por um evento que o scan não confirmou é o erro que o produto inteiro existe para evitar.
- **O piso de orçamento é aritmética, não opinião.** A fase de aprendizado precisa de ~50 eventos de otimização em 7 dias (documentação oficial, trust 1). Com ticket, margem e CAC alvo declarados no contexto, o orçamento diário mínimo por conjunto é **calculável**. É a informação mais valiosa que se pode dar a um iniciante — e é também o portão honesto: *"com R$X/dia você não sai do aprendizado otimizando para compra; otimize por um evento de topo, aumente o orçamento, ou não comece ainda"*.
- **O número de conjuntos** = orçamento total ÷ esse piso. Mata o erro mais comum de iniciante — fragmentar a verba até nenhum conjunto aprender — com conta, não com regra de bolso.
- **A alocação de criativos** sai das hipóteses da fase 8, já etiquetadas na taxonomia canônica.
- **A janela de julgamento e o "não mexa"** saem das docs trust 1: quantos dias antes de qualquer decisão, o que reinicia o aprendizado, o que é ruído estatístico.
- **O remarketing entra aqui, como etapa 2, com a pré-condição declarada** — "você só terá público para remarketing depois de N visitantes; espere isso por volta do dia X". Deixa de ser um checkbox impossível na Prontidão e vira etapa com precedência explícita.

**Contrato de saída** (além dos campos de honestidade comuns): o evento de otimização escolhido **com a justificativa do porquê dele e não de outro**; o piso de orçamento diário por conjunto **com a conta aberta** (eventos necessários × custo por evento derivado do CAC alvo); a estrutura recomendada (quantas campanhas, quantos conjuntos, postura de segmentação — amplo vs. detalhado — e quantos criativos por conjunto, vindos das hipóteses existentes); as **etapas em ordem com pré-condição declarada** (o que lançar agora, o que só faz sentido depois de qual sinal); a janela de julgamento e a lista do que **não** tocar enquanto ela corre; e um critério de sucesso relativo à própria conta.

**Invariantes (inegociáveis):**

- **Prescreve, nunca opera.** Este é o ponto do produto mais próximo de "operar" — a linha é que o plano diz o que configurar e com que números, e **nunca toca na conta**. Não existe botão que crie campanha.
- **Piso de orçamento é uma exigência declarada, não uma promessa de resultado.** O motor nunca prevê CPA, ROAS ou faturamento. Ele diz o que é preciso gastar para *conseguir aprender* — quanto vai custar aprender, não quanto vai render.
- **Degrada honesto.** Se a economia declarada não sustenta um teste viável, a saída correta é *"não comece ainda, e por quê"* mais o que mudaria isso (ticket, margem, evento de otimização mais raso, orçamento) — nunca um plano fabricado para não frustrar.
- **Não é portão.** Quem quiser ignorar e anunciar do jeito que preferir, anuncia — terá sido avisado, com a conta na mesa.
- **Não depende de conta Meta conectada.** É zero-data como a Prontidão: roda do contexto do produto + conhecimento. A conexão enriquece depois (planejado × real), nunca é pré-requisito.
- **A Prontidão é input, não pré-requisito.** Sem veredito, o plano usa o declarado e diz em `missing_data` o que a Prontidão resolveria — e nunca recomenda otimizar por um evento não confirmado sem marcar isso como o risco número um.
- **Sem benchmark inventado.** Nada de "o CTR médio do setor é X". Critério de sucesso é sempre relativo à própria conta ou à documentação oficial citada.

**O laço fecha como todo o resto:** cada etapa do plano materializa, sob demanda, um experimento rastreado no journal (mesma idempotência por `(diagnosis_id, change_made)`), e o diagnóstico de campanha passa a receber o plano no briefing — o que transforma a primeira leitura de dados pagos em **planejado × real** em vez de uma foto sem passado. É a ponte natural entre o mundo pré-pago e os cinco pilares.

**Correções na fase 7 que vêm com esta fase** (o vazamento e sua causa): `remarketingAudience` sai do checklist de Prontidão (ficam `emailCapture` e `emailFollowup`, que são estrutura que o negócio possui e existe com zero Meta); "remarketing" sai da definição da dimensão `funil` e "estrutura mínima para a primeira campanha" sai de `midia`; e o prompt do motor de prontidão ganha uma **regra de escopo explícita** — nunca prescrever configuração dentro da plataforma (públicos, estrutura de campanha, orçamento, lance); quando um achado depender disso, nomear como próximo passo e apontar o Plano de Lançamento, jamais como tarefa da Prontidão. Sem essa regra o vazamento volta: o motor tem as docs da Meta na recuperação e puxa mecânica de plataforma naturalmente.
...
| 7 | **Camada de Prontidão (pré-tráfego)**: modo "prontidão" do motor (mesma engine + formato fixo, grounding no `growth-playbook`), intake estrutural leve reusando o contexto do produto, veredito explicável na Home / pós-cadastro; scanner técnico da URL como enriquecimento opcional (fase B) | **fase A implementada** (2026-07-21) — migration `0028` (tabela `product_readiness` + `diagnoses.scope = 'readiness'` + assistente `readiness-engine`), UI `/readiness` (checklist declarado → veredito priorizado por alavancagem, com esforço × impacto e critério de sucesso por achado), **bloqueadores determinísticos calculados localmente** (grátis, sem LLM, visíveis enquanto o usuário marca) e sequenciamento da jornada (prontidão antes do diagnóstico no card pós-cadastro e no checklist de ativação). É **zero-data** e reusa motor + `growth-playbook` + modelo de cobrança — nenhum domínio novo. **Fase B também implementada** (2026-07-21): migration `0029` (`product_scans`, série temporal — scans que falharam são preservados, pois "sua página não respondeu" é um achado), `lib/readiness/scan.ts` (fetch com **guarda SSRF**: só http/https, só portas padrão, resolução DNS com rejeição de loopback/privado/link-local/CGNAT/IPv4-mapped, redirecionamentos seguidos manualmente e revalidados a cada salto, timeout e teto de bytes) + `scan-analyze.ts` puro, bloco de scan no briefing e card `/readiness` que exibe observado ao lado de declarado. Suíte `npm run test:readiness` (928 asserções em 2026-07-31) cobre avaliador, guarda SSRF, analisador, verificação por camadas (GTM/SPA), PageSpeed, diff de vereditos, cooldown, contrato de erros e cobertura i18n. **Fase C também implementada**: laço fechado (achado → experimento rastreado, por achado, idempotente por `(diagnosis_id, change_made)` — sem migration nova) e presença orgânica real no briefing (conteúdos vinculados + Review), como contexto da descoberta e nunca como atribuição. **Fase D (2026-07-30)**: passo a passo sob demanda (`0031`), concierge merecido (`0032`), modelo de aquisição + dimensão `ativacao` (`0034`), teto de tokens do veredito dobrado com reasoning limitado a 1/3 do orçamento (`0037`, renomeada de um arquivo `0035` duplicado que não derivava marcador — o runner agora FALHA alto em migração sem marcador), cobrança transacional (`0038`/`0039`), lock de geração (`0040`), **fronteira de escopo (`0041`)**, **localização da correção (`0043`/`0044`)** — `stage` + `where` obrigatórios, as regras de "só conheço as telas que me contaram" e "não concluo sobre o que não li", e o fim do vazamento da chave interna na evidência — e **chaves do checklist com fonte única (`0042`)** — `related_items` virou enum GERADO de `READINESS_ITEM_KEYS` e escopado pelo modelo de funil (venda direta não recebe chave de ativação), o bloco do checklist passou a carregar a chave ao lado do rótulo, e a enumeração em prosa saiu do prompt: era o mesmo fato em três lugares, e a `0034` já tinha derivado ao acrescentar 4 chaves sem tocar no prompt — — `remarketingAudience` sai do checklist e o motor fica proibido de prescrever configuração dentro do Gerenciador (públicos, estrutura de campanha, orçamento, lance): isso é trabalho da fase 9 e, antes do primeiro gasto, é literalmente impossível de cumprir —, `next_review_at` + cron ciente de escopo, memória de experimentos e evidência criativa no briefing, PageSpeed Insights opcional, e a jornada UX (histórico com diff, deep links, wizard retomável, portas nos bloqueadores, sinais do concierge persistidos). Migrations da prontidão: `0028`, `0029`, `0031`, `0032`, `0034`, `0037`–`0044` — **todas aplicadas no banco de desenvolvimento** (conferido em 2026-07-31 com `npm run db:migrate -- --dry-run`: "Database is up to date" — inclusive a `0037`, cujo arquivo antigo era pulado em silêncio) |

| 8 | **Plano de Teste Criativo (pré-pago)**: modo "creative_plan" do motor (mesma engine + formato fixo, grounding em `growth-playbook` + `meta-ads-docs`), hipóteses em slugs canônicos com prompt copiável, materialização hipótese→criativo etiquetado (`creative_plan_links`) e hipótese→experimento, cobertura verificada por máquina na superfície de Criativos, sinais vivos na `/library`, bloco de evidência criativa no briefing do diagnóstico pago | **implementada** (2026-07-28) — migration `0033` (tabela `creative_plan_links` + `diagnoses.scope = 'creative_plan'` + assistente `creative-plan-engine`, 3 créditos), lib `apps/web/src/lib/creative-plan/`, ação `generateCreativePlan`, suíte `npm run test:creative-plan` (25 asserções). **Migration `0033` aplicada** (conferido em 2026-07-31 via `npm run db:migrate -- --dry-run`) |
| 9 | **Plano de Lançamento (a ponte pré-pago → pago)**: modo `launch_plan` do motor (mesma engine + formato fixo, grounding em `meta-ads-docs` trust 1 + `growth-playbook`) — evento de otimização herdado do que a Prontidão PROVOU disparar, piso de orçamento diário calculado a partir do requisito de fase de aprendizado × economia declarada, nº de conjuntos derivado do piso, alocação de criativos vinda das hipóteses da fase 8, etapas com pré-condição declarada (remarketing como etapa 2), janela de julgamento e lista do "não mexa". Materializa etapa→experimento e entra no briefing do diagnóstico pago como **planejado × real**. Inclui as correções de escopo da fase 7 (sai `remarketingAudience` do checklist; saem "remarketing" de `funil` e "estrutura mínima para a primeira campanha" de `midia`; entra a regra de escopo que proíbe o motor de prontidão prescrever configuração dentro da plataforma) | **implementada** (2026-08-06) — fecha o penhasco entre "pode anunciar" (fase 7/8) e "vamos analisar suas campanhas" (fase 3). Migration `0046` (tabela `launch_plan_run_locks` + `diagnoses.scope = 'launch_plan'` + assistente `launch-plan-engine`, 3 créditos) **aplicada no banco de desenvolvimento**. O evento de otimização e o piso de orçamento são **calculados no servidor** (`lib/launch-plan/math.ts`), nunca pelo modelo — o sanitizador (`sanitizeLaunchPlan`) sobrescreve o que o modelo escrever nesses campos, o mesmo princípio que protege a taxonomia canônica da fase 8. `optimization_event.basis` distingue honestamente `proved` (Pixel provado pelo scan + evento declarado testado) de `declared` e `missing` — e uma base fraca é sempre o risco nº 1 do plano, nunca letra miúda. UI em `/launch` e `/products/[id]/launch`, nova etapa 4 no percurso do workspace (rail, Home, next-step-card, checklist de ativação). Suíte `npm run test:launch-plan` (128 asserções) cobre a matemática, o sanitizador, os blocos do briefing e a paridade i18n dos códigos de erro nos 5 idiomas |
