...
| **Layout archetypes** | Hero `layout="split"` with the **mono `DiagnosisReadout`** as evidence (not browser chrome); an **instrument pipeline** section directly below (`ProcessSteps mono`); `StatBand` with oversized numbers; `FeatureRows` zig-zag with the domain vignettes (`DiagnosisVignette`/`FatigueVignette`); `BentoGrid` for the remaining pillars (`FunnelVignette`/`MemoryVignette`). Never centered-stack + equal-card-grid all the way down. |
| **Signature element** | The **mono diagnosis readout** — the product's fixed recommendation format rendered as an instrument panel above the fold. A naked text hero is a direction violation. |
| **Motion** | One orchestrated hero timeline (staggered copy + readout rise via GSAP `useGSAP`); quiet `<Reveal>` elsewhere; chart/line draw-ins in the vignettes. No uniform scroll-reveals on everything. |
...
- The home implements this direction end to end (instrument hero + pipeline, oversized StatBand, FeatureRows with vignettes, BentoGrid). It is the reference implementation new pages read.
